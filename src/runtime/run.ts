...
import type { ChatMessage, ModelAdapter } from '../models/types.js';
...
export type RunEvent =
  ...
  | { type: 'tool.result'; turn: number; name: string; ok: boolean }
...
export async function runSkill(options: RunOptions): Promise<{ skill: LoadedSkill; output: string; turns: number }> {
  ...
  options.onEvent?.({ type: 'skill.loaded', skill: skill.name, skillFile: skill.skillFile });
  ...
  for (let turn = 1; turn <= maxTurns; turn++) {
    options.onEvent?.({ type: 'model.call', turn, model: options.model.id });
    const response = await options.model.chat(messages, tools);
    messages.push({ role: 'assistant', content: response.content, tool_calls: response.toolCalls });

    if (!response.toolCalls.length) {
      options.onEvent?.({ type: 'run.completed', turns: turn });
      return { skill, output: response.content, turns: turn };
    }

    for (const call of response.toolCalls) {
      options.onEvent?.({ type: 'tool.call', turn, name: call.name, arguments: call.arguments });
      const result = await executeTool(call.name, call.arguments, providers);
      options.onEvent?.({ type: 'tool.result', turn, name: call.name, ok: result.ok });
